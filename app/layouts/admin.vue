      ...
      <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center space-x-3">
          <div class="w-8 h-8 bg-red-600 rounded-lg flex items-center justify-center">
            <span class="text-white font-bold text-sm">관</span>
          </div>
      ...
      <nav class="mt-4 px-4">
        <div class="space-y-2">
          <NuxtLink
            ...
            :class="{ 'bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-400': $route.path === '/admin' }"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
            </svg>
            <span v-if="sidebarOpen || sidebarPinned">조직 관리</span>
          </NuxtLink>

          <NuxtLink
            to="/admin/todos"
            class="flex items-center space-x-3 px-3 py-2 rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
            :class="{ 'bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-400': $route.path === '/admin/todos' }"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
            <span v-if="sidebarOpen || sidebarPinned">시스템 숙제</span>
          </NuxtLink>

          <NuxtLink
            to="/admin/users"
            class="flex items-center space-x-3 px-3 py-2 rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
            :class="{ 'bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-400': $route.path === '/admin/users' }"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z" />
            </svg>
            <span v-if="sidebarOpen || sidebarPinned">사용자 관리</span>
          </NuxtLink>

          <NuxtLink
            to="/admin/system-account"
            class="flex items-center space-x-3 px-3 py-2 rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
            :class="{ 'bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-400': $route.path === '/admin/system-account' }"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            <span v-if="sidebarOpen || sidebarPinned">시스템 계정 생성</span>
          </NuxtLink>
          ...
          <div class="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4">
            ...
            >
              ...
              <span v-if="sidebarOpen || sidebarPinned">일반 대시보드</span>
            </NuxtLink>
          </div>
        </div>
      </nav>
        ...
        <div class="flex items-center justify-between px-4 py-3">
          ...
          <div class="flex items-center space-x-4">
            ...
            <div class="flex items-center space-x-2">
              <h1 class="text-xl font-semibold text-gray-900 dark:text-white">관리자 패널</h1>
            ...
            <div ref="userMenuRef" class="relative">
              ...
              >
                <div class="w-8 h-8 bg-red-600 rounded-full flex items-center justify-center">
                  <span class="text-white text-sm font-medium">{{ userStore.user?.name?.charAt(0) || 'A' }}</span>
                </div>
                <span class="hidden md:block text-sm font-medium">{{ userStore.user?.name || '관리자' }}</span>
              ...
              >
                <div class="px-4 py-2 border-b border-gray-200 dark:border-gray-700">
                  <p class="text-sm font-medium text-gray-900 dark:text-white">{{ userStore.user?.name }}</p>
                  <p class="text-sm text-gray-500 dark:text-gray-400">{{ userStore.user?.email }}</p>
                  <p class="text-xs text-red-600 font-medium">시스템 관리자</p>
                </div>
                <NuxtLink
                  to="/dashboard"
